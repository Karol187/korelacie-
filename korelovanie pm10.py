# -*- coding: utf-8 -*-
"""
Created on Sat Jan 18 13:05:04 2020
...
for key,value in dic_polia.items():
    #DF=pd.read_csv('tabulka_surova_PM10.csv',delimiter=',')
    zoznam_vsetky=[]
    zoznam_UB_RB_SB_SI=[]
    zoznam_UT_RB_SB_SI=[]
    zoznam_UB_UT_SB_SI=[]
...
##################################################################################################              
#        DF[key]=z
#        korelacia_vsetky=np.log(DF['PM10']).corr(DF[key])
#        zoznam_vsetky.append(korelacia_vsetky)
#        DF_filteredUT = DF[(DF.typ != 'UT')]
#        korelacia_UB_RB_SB_SI=np.log(DF_filteredUT['PM10']).corr(DF_filteredUT[key])
#        zoznam_UB_RB_SB_SI.append(korelacia_UB_RB_SB_SI)
#        DF_filteredUB = DF[(DF.typ != 'UB')]
#        korelacia_UT_RB_SB_SI=np.log(DF_filteredUB['PM10']).corr(DF_filteredUB[key])
#        zoznam_UT_RB_SB_SI.append(korelacia_UT_RB_SB_SI)
#        DF_filteredRB = DF[(DF.typ != 'RB')]
#        korelacia_UB_UT_SB_SI=np.log(DF_filteredRB['PM10']).corr(DF_filteredRB[key])
#        zoznam_UB_UT_SB_SI.append(korelacia_UB_UT_SB_SI)
#        DF_filteredSI = DF[(DF.typ != 'SI')]
#        korelacia_UB_UT_RB_SB=np.log(DF_filteredSI['PM10']).corr(DF_filteredSI[key])
#        zoznam_UB_UT_RB_SB.append(korelacia_UB_UT_RB_SB)
#        DF_filteredSB = DF[(DF.typ != 'SB')]
#        korelacia_UB_UT_RB_SI=np.log(DF_filteredSB['PM10']).corr(DF_filteredSB[key])
#        zoznam_UB_UT_RB_SI.append(korelacia_UB_UT_RB_SI)
#        DF_filter = DF[(DF.typ != 'SI')& (DF.typ != 'SB')& (DF.typ != 'RB')& (DF.typ != 'UT')]
#        korelacia_UB = np.log(DF_filter['PM10']).corr(DF_filter[key])
#        zoznam_UB.append(korelacia_UB)
#        DF_filter1 = DF[(DF.typ != 'SI')& (DF.typ != 'SB')& (DF.typ != 'RB')& (DF.typ != 'UB')]
#        korelacia_UT=np.log(DF_filter1['PM10']).corr(DF_filter1[key])
#        zoznam_UT.append(korelacia_UT)       
##################################################################################################
#KORELACIE S PM10
##################################################################################################
        DF[key]=z
        korelacia_vsetky=DF['PM10'].corr(DF[key])
        zoznam_vsetky.append(korelacia_vsetky)
        DF_filteredUT = DF[(DF.typ != 'UT')]
        korelacia_UB_RB_SB_SI=DF_filteredUT['PM10'].corr(DF_filteredUT[key])
        zoznam_UB_RB_SB_SI.append(korelacia_UB_RB_SB_SI)
        DF_filteredUB = DF[(DF.typ != 'UB')]
        korelacia_UT_RB_SB_SI=DF_filteredUB['PM10'].corr(DF_filteredUB[key])
        zoznam_UT_RB_SB_SI.append(korelacia_UT_RB_SB_SI)
        DF_filteredRB = DF[(DF.typ != 'RB')]
        korelacia_UB_UT_SB_SI=DF_filteredRB['PM10'].corr(DF_filteredRB[key])
        zoznam_UB_UT_SB_SI.append(korelacia_UB_UT_SB_SI)
        DF_filteredSI = DF[(DF.typ != 'SI')]
        korelacia_UB_UT_RB_SB=DF_filteredSI['PM10'].corr(DF_filteredSI[key])
        zoznam_UB_UT_RB_SB.append(korelacia_UB_UT_RB_SB)
        DF_filteredSB = DF[(DF.typ != 'SB')]
        korelacia_UB_UT_RB_SI=DF_filteredSB['PM10'].corr(DF_filteredSB[key])
        zoznam_UB_UT_RB_SI.append(korelacia_UB_UT_RB_SI)
        DF_filter = DF[(DF.typ != 'SI')& (DF.typ != 'SB')& (DF.typ != 'RB')& (DF.typ != 'UT')]
        korelacia_UB = DF_filter['PM10'].corr(DF_filter[key])
        zoznam_UB.append(korelacia_UB)
        DF_filter1 = DF[(DF.typ != 'SI')& (DF.typ != 'SB')& (DF.typ != 'RB')& (DF.typ != 'UB')]
        korelacia_UT=DF_filter1['PM10'].corr(DF_filter1[key])
        zoznam_UT.append(korelacia_UT)       
...
################################################################################################        
    ...
    tabulka=pd.DataFrame({'oblast km^2': plocha,'Vsetky':zoznam_vsetky, 'UB_RB_SB_SI':zoznam_UB_RB_SB_SI, 'UT_RB_SB_SI':zoznam_UT_RB_SB_SI,'UB_UT_SB_SI':zoznam_UB_UT_SB_SI, 'UB_UT_RB_SB':zoznam_UB_UT_RB_SB,'UB_UT_RB_SI':zoznam_UB_UT_RB_SI, 'zoznam_UB':zoznam_UB,'zoznam_UT':zoznam_UT})
    
    tabulka_zaokruhlena=tabulka.round({'Vsetky': 3, 'UB_RB_SB_SI': 3,'UT_RB_SB_SI':3,'UB_UT_SB_SI':3,'UB_UT_RB_SB':3,'UB_UT_RB_SI':3,'zoznam_UB':3,'zoznam_UT':3})
    #mala tabulka 
    primerna_korelacia_vsetky=statistics.mean(tabulka['Vsetky'])
    primerna_korelacia_UB_RB_SB_SI=statistics.mean(tabulka['UB_RB_SB_SI'])
    primerna_korelacia_UT_RB_SB_SI=statistics.mean(tabulka['UT_RB_SB_SI'])
    primerna_korelacia_UB_UT_SB_SI=statistics.mean(tabulka['UB_UT_SB_SI'])
    primerna_korelacia_UB_UT_RB_SB=statistics.mean(tabulka['UB_UT_RB_SB'])
    primerna_korelacia_UB_UT_RB_SI=statistics.mean(tabulka['UB_UT_RB_SI'])
    ...
    data = [['Vsetky',primerna_korelacia_vsetky ], ['UB_RB_SB_SI', primerna_korelacia_UB_RB_SB_SI], ['UT_RB_SB_SI', primerna_korelacia_UT_RB_SB_SI],['UB_UT_SB_SI', primerna_korelacia_UB_UT_SB_SI], ['UB_UT_RB_SB', primerna_korelacia_UB_UT_RB_SB], ['UB_UT_RB_SI', primerna_korelacia_UB_UT_RB_SI],['zoznam_UB', primerna_korelacia_zoznam_UB], ['zoznam_UT', primerna_korelacia_zoznam_UT]]  
